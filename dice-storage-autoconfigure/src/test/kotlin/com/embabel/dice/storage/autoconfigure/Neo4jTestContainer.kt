 ...
 * (no override hook of its own, and 0.0.58 is drivine4j's newest release -- see
 * `Neo4jTestcontainerVersionOverride` for that whole story). `5.26.1-community` has a confirmed
 * upstream bug (https://github.com/neo4j/neo4j/issues/13597): a dynamic relationship-type
 * parameter gets baked into the query-plan cache on first execution and silently reused for every
 * later execution of the same query text with a different value. Confirmed fixed on
 * `neo4j:2026.05-community`, which is what this starts.
 *
