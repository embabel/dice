...
| `mcp` | `DiceMcpTools` — simplified MCP tool surface (`dice_recall`, `dice_list`, `dice_store`, `dice_get`); `contextId` on every call is a caller-supplied scope (not a credential) so one call cannot read another context; authorization is the host MCP server's job |
