...
| `mcp` | `DiceMcpTools` — simplified MCP tool surface (`dice_recall`, `dice_list`, `dice_store`, `dice_get`); `context_id` on every call so a stateless client cannot cross tenants |
