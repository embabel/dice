...
This note covers DICE #66: the two reference types (`ExtractionContentProfileRef` and
`ExtractionRunRef`), where they sit on `SourceAnalysisContext` and the extraction entry points,
and why profile, perspective, schema and tenant are four independent dimensions rather than one
knob with four names.
...
Neither is parsed. DICE compares them, carries them, and stores them (from #67 onward). The
alternative — DICE resolving a profile into an actual configuration — costs three things it has
no business owning:
...
makes that decision on its own side, before it calls DICE, and passes the reference along so the
resulting claims can be attributed to the policy they were extracted under.
...
`buildContext` reads either reference; a test compares the whole context built with a profile
against the one built without and asserts they differ in exactly that field.

Identity is name *and* version together. A host that republishes `house-style` as `v2` gets a
distinct reference, so runs attributed to `v1` stay attributed to `v1` and a later comparison of
the two is a comparison of two identities rather than an archaeology problem.

Both strings are bounded — 256 characters for a name, 64 for a version — because #67 stores them
in run headers and indexes them. A reference is an identifier the host mints, not a place to put
a payload.

## Why the run reference ships before the run

`ExtractionRunRef` is a single opaque id and nothing else: no timing, no status, no counts, no
lineage.
...
Durable extraction runs are DICE #67, a whole wave behind this one. The run model will be keyed
by (`ContextId`, `ExtractionRunRef`). Shipping the reference now means the entry points and the
run model meet at an opaque string rather than at a type one of them has to import from the
other's release: this slice can carry a run through extraction while #67 is still being designed,
and #67 can key its store on a type that already exists and is already tested.

Carrying a reference is always allowed, and DICE does not check that the run exists — there is
nowhere yet to check against. What a store does with a reference to a run it has never seen is
#67's decision; this slice makes no promise about it in either direction, so #67 stays free to
require referential integrity when it wires proposition-to-run attribution. DICE mints no run ids
here either: a run is something the host starts, or, from #67's wiring slice, something DICE's own
run coordinator starts.

Two properties the reference commits to now, because they are cheaper to hold than to retrofit:

- **It is not part of source-provenance equality.** Two runs over the same material still produce
  one piece of source evidence. What differs is which runs are attributed to it, which is a
  separate relation in #67 rather than a field on the evidence.
- **It is a pseudonymous token.** Not an authorization token, not a direct identifier, and not
  something a reader can dereference into personal data. Length-capped at 256 characters for the
  same reason the profile strings are.
...
flowchart TD
    ...
    CTX --> R["currentRun<br/>ExtractionRunRef?<br/><i>which execution this was</i>"]
...
Profile and run are checked against nothing, and the `init` block says so — validating them
against another dimension would invent a relationship the contract does not have.
...
A fourth case pins the copy helpers by comparing `withProfile(p)` against `copy(profile = p)`,
which is a statement about all thirteen components at once rather than about the one field the
helper names.
...
flowchart TD
    ...
    B["SourceAnalysisRequestEvent.profile() / currentRun()"] --> C
    C["IncrementalPropositionExtraction.buildContext"] --> CTX
    CTX["SourceAnalysisContext.profile<br/>SourceAnalysisContext.currentRun"] --> X["carried, never consulted"]
```

| Path | How a profile arrives |
| --- | --- |
| `rememberText`, `rememberTextFromSource` | trailing optional `profile` and `currentRun` arguments |
| `rememberFile`, `rememberFileFromSource` | the same two arguments, forwarded to the text call |
| async `SourceAnalysisRequestEvent` | `profile()` / `currentRun()`, both open and null-defaulted |
...
The direct calls take extra arguments rather than getting their own method names, which is the
...
Growing the existing declarations with two defaulted parameters would have been the obvious
shape, and it is wrong. `@JvmOverloads` emits every reduced-arity overload as `final`, even on an
`open` function — only the declared maximum arity stays open. So adding `profile` and `currentRun`
to `rememberText` would have moved the open declaration from six arguments to eight and re-emitted
the six-argument form as a final bridge. Callers would not have noticed. A subclass overriding the
six-argument form would have stopped compiling, and one already compiled could fail verification
at class load.
...
two references and carries no defaults. The defaults are what force the split — two overloads that
both supply defaults for the same arity are ambiguous at a Kotlin call site, so the wide form
spells every argument out.

| Method | Overridable before | Overridable now |
| --- | --- | --- |
| `rememberText` | 6 args | 6 args, and 8 |
| `rememberTextFromSource` | 8 args | 8 args, and 10 |
| `rememberFile` | 3 args | 3 args, and 5 |
| `rememberFileFromSource` | 5 args | 5 args, and 7 |
...
So the rule is that a call dispatches like the call it resembles. A file call carrying no
references takes the pre-profile chain; one carrying a reference has to go wide, because the
legacy text signature cannot express a profile.

```mermaid
flowchart LR
    F3["rememberFile(3)"] --> T6
    F5["rememberFile(5)"] -->|"both refs null"| F3
    F5 -->|"a ref is present"| T8
    T6["rememberText(6)"] --> T8["rememberText(8)"] --> I["rememberTextInternal"]
```
...
The async path reads each accessor exactly once, which a test asserts by counting: there is one
...
- Every Java-visible `SourceAnalysisContext` constructor descriptor that existed before this slice
  is still published. `@JvmOverloads` adds the two new ones on the end. A test enumerates arities
  3 through 13 (each with the trailing `DefaultConstructorMarker` Kotlin emits because `contextId`
...
- Every `rememberText`, `rememberTextFromSource`, `rememberFile` and `rememberFileFromSource`
  descriptor survives, and the new arguments add exactly one descriptor per method name, on the
  end. A test asserts the exact descriptor set of all four names, that a profile and a run always
  arrive together, and that they are always the last two parameters.
...
- `ConversationAnalysisRequestEvent` keeps its five-argument constructor and gains six- and
  seven-argument forms. Its `sourceLocator` parameter relaxes from non-null to nullable, which
  accepts strictly more calls than before.
- **Full Kotlin synthetic `copy` and `componentN` ABI is not claimed.** Adding two fields to a
  data class rewrites `copy` and adds two `componentN` methods, so Kotlin code compiled against an
  earlier jar must be recompiled rather than swapped in. This is the same half of the boundary
  #64 declined, for the same mechanical reason, and a test pins it: exactly one `copy` remains and
  it takes thirteen arguments.
- No stored data changes. Nothing serializes a profile or a run reference yet; #67 is where they
  reach a store.

## Status: EXPERIMENTAL

`ExtractionContentProfileRef` and `ExtractionRunRef` both carry `@ApiStatus.Experimental`, which
is the marker DICE already uses for API that may still move (`PrologProjector`,
`PropositionStatus.STALE`). The KDoc on both types and on every new parameter says the same in
words, and the CHANGELOG entry is labelled.

The shape most likely to move is `ExtractionRunRef`, because #67 is where a run acquires
everything else about itself and may want the reference to carry more than one string.
...
- **No host profile catalog.** DICE holds no mapping from a reference to a policy and offers no
  place to put one.
- **No durable run.** `ExtractionRunRef` names a run; nothing stores one until #67.
...
- **No behaviour.** Extraction, resolution and revision ordering are behaviour-identical. A
  profile changes what a run is *attributed to*, not what it does.
